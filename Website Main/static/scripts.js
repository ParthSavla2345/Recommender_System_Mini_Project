...
// Function to validate the destination input field
function validateDestination() {
  const destinationInput = document.getElementById('destination');
  const destinationError = document.getElementById('dest_err');
  const destinationValue = destinationInput.value.trim();
  const getRecommendationsBtn = document.getElementById('getRecommendationsBtn');

  // Regular expression to check if the input contains only letters, spaces, and certain special characters
  const textRegex = /^[a-zA-Z\s,.'"-]+$/;

  if (destinationValue === '') {
    // If the input field is empty, clear the error message, reset the border, and disable the button
    destinationError.textContent = '';
    destinationInput.style.border = '3px solid #ccc';
    // getRecommendationsBtn.disabled = true;
  } else if (!textRegex.test(destinationValue)) {
    // If the input value contains numbers or invalid characters
    destinationError.textContent = 'Please enter text only!';
    destinationInput.style.border = '4px solid red';
    // getRecommendationsBtn.disabled = true;
  } else {
    // If the input value is valid text
    destinationError.textContent = '';
    destinationInput.style.border = '3px solid #ccc';
  }
}
...
function validateDays() {
  ...
  const getRecommendationsBtn = document.getElementById('getRecommendationsBtn');
  const daysError = document.getElementById('days_err');

  if (daysValue === '') {
    daysError.textContent = '';
    daysInput.style.border = '3px solid #ccc';
  } else if (daysValue < 2) {
    daysError.textContent = 'Number of days should be at least 2';
    daysInput.style.border = '4px solid red';
    // getRecommendationsBtn.disabled = true;
  } else {
    daysError.textContent = '';
    daysInput.style.border = '3px solid #ccc';
...
// Add event listener to the destination input field
const destinationInput = document.getElementById('destination');
destinationInput.addEventListener('input', validateDestination);
const daysInput = document.getElementById('days');
daysInput.addEventListener('input', validateDays);

function clr_err(inputId) {
  const inputField = document.getElementById(inputId);
  const errorElement = document.getElementById(`${inputId}_err`);
  const getRecommendationsBtn = document.getElementById('getRecommendationsBtn');
  ...
  if (inputId === 'hotel-price' && inputField.value < 500) {
    errorElement.textContent = 'Hotel price should not be less than 500.';
    inputField.style.border = '4px solid red';
    // getRecommendationsBtn.disabled = true;
  }
}
